...
{
  int len = dim*dim;
  int arr_size = len*sizeof(float);
  float *h_dist;

  int i, j;

  h_dist = new float[len];

  for (i = 0; i < dim; ++i)
  {
    for (j = 0; j < dim; ++j)
    {
      h_dist[i*dim+j] = (float) sqrt((float)pow(city[i][0] - city[j][0], 2)
                                    +(float)pow(city[i][1] - city[j][1], 2));
    }
  }
...
float getDist(int *route, float *distArr, int dim)
{
  int i;
  float total_dist = 0;
  ...
  for (i = 0; i < dim-1; ++i)
  {
    total_dist += distArr[route[i] * dim + route[i+1]];
  }
  total_dist += distArr[route[dim-1] * dim + route[0]];
  return total_dist;
}
...
int* randRoute(int dim)
{
  ...
  // std::random_shuffle(&route[0], &route[dim-1]);
